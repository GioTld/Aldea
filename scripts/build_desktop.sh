...
go build -tags desktop -o "${OUTPUT_DIR}/aldea-desktop-linux-amd64" ./gui

# 2. Windows AMD64 Target Binary
echo "[+] Compiling Windows Target Binary..."
GOOS=windows GOARCH=amd64 go build -tags desktop -o "${OUTPUT_DIR}/aldea-desktop-windows-amd64.exe" ./gui

# 3. macOS ARM64 Target Binary
echo "[+] Compiling macOS Target Binary..."
GOOS=darwin GOARCH=arm64 go build -tags desktop -o "${OUTPUT_DIR}/aldea-desktop-darwin-arm64" ./gui
